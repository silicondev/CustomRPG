...
public class CommandCRPG {
	...
	String permNode;
	List<CommandCRPG> children = new ArrayList<CommandCRPG>();
	CommandCRPG parent;
	boolean canChildren = false;
	
	public CommandCRPG(String i, int rNum, int oNum, boolean playOnly, boolean cchildren, boolean hParent, int outID, String perm, String desc) {
		inputName = i;
		description = desc;
		reqParams = rNum;
		if (oNum == -1) {
